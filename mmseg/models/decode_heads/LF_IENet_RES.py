...
class CrossFrameAttention(nn.Module):
    ...
    def forward(self, memory_keys, memory_values, query_query, disparity, sequence_index):
        ...
        for i in range(sai_number):   
            ...
            atten_mask[total_disparity > 0.25 * self.radius] = weight * 0.25;
            atten_mask[total_disparity < -0.25 * self.radius] = weight * 0.25;

            atten_mask[total_disparity > 0.5 * self.radius] = weight * 0.5;
            atten_mask[total_disparity < -0.5 * self.radius] = weight * 0.5;

            atten_mask[total_disparity > self.radius] = weight
            atten_mask[total_disparity < -1 * self.radius] = weight

            atten_mask[total_disparity > 2 * self.radius] = weight * 2
            atten_mask[total_disparity < -2 * self.radius] = weight * 2

            atten_mask[total_disparity > 4 * self.radius] = weight * 4;
            atten_mask[total_disparity < -4 * self.radius] = weight * 4;
...
class LF_IENET_RES(BaseDecodeHead):
    def __init__(self, sai_number, key_channels, value_channels, **kwargs):
        super(LF_IENET_RES, self).__init__(**kwargs)
    ...
    def forward(self, inputs, sequence_imgs, disparity, sequence_index):
        ...
        # 用自注意力机制对参考视图提取特征
        encoding_feature = [None] * sai_number
        query = [None] * sai_number
        key = [None] * sai_number
        value = [None] * sai_number
        feature = [None] * sai_number
        for i in range(sai_number):
            encoding_feature[i] = self.center_encoding(sequence_imgs[i])
            query[i] = self.center_query_conv(encoding_feature[i])
            key[i] = self.center_key_conv(encoding_feature[i])
            value[i] = self.center_value_conv(encoding_feature[i])

        for i in range(sai_number):
            feature[i] = self.self_attention(query[i], key[i], value[i]) + reference_encoding_feature[i]

        reference_key = self.reference_key_conv(feature)
        reference_value = self.reference_value_conv(feature)
        ...
        disparity_8 = disparity_8.permute(0, 2, 3, 1).contiguous()      
        ...
        coords = coords[0, xy_position[:, :, :, 0].reshape(-1).to(torch.int64),xy_position[:, :, :, 1].reshape(-1).to(torch.int64), :]    # 基于裁剪的patchsize像素块选出对应的coords值
        ...
        for i in range(sai_number):                     
            ...
            offsetx = 2 * (dst_u - sequence_index[:,i,1]).reshape(batch_size,1,1,1) * disparity_8[:, :, :, :]     
            offsety = 2 * (dst_v - sequence_index[:,i,0]).reshape(batch_size,1,1,1) * disparity_8[:, :, :, :]     
